"""Inject prompt/spec data into the reusable fixed eaglePolicy Java template."""
...
def _render_spec_block(spec: dict) -> str:
    """Render one eaglePolicy strategy spec into the Java constant block."""
...
def render_eagle_policy_agent(repo_root: Path, prompt: str, spec: dict) -> Path:
    """Reuse `eaglePolicy.java` by injecting one prompt and policy spec."""
    microrts_root = locate_microrts_root(repo_root)
    java_path = microrts_root / "src" / "ai" / "abstraction" / "eaglePolicy.java"
    ...
    if spec_match is None or prompt_match is None:
        ...
        raise ValueError(f"Failed to locate eaglePolicy {missing_text} markers in {java_path}")
