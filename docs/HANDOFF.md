...
The initial prompt starts from the known-good MicroRTS `ai.PassiveAI` no-op behavior. Future generated agents should edit only the `chooseAction` strategy body while the scaffold keeps imports, class shell, `getAction`, `clone`, and parameters fixed.
