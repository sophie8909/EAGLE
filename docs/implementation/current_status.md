...
Snapshot: 2026-07-14 after Migration Phase 1. This file describes active source/tests/configuration. It is not normative. Recent retained runs predate the current complete-file implementation and are treated as legacy format evidence only.
...
| Mutation | Two prompt pairs exist. Default `RuleBasedMutationBackend` makes two local calls and returns unchanged prompt content. | Reflection/Rewrite shapes exist but are not LLM calls, are not durably logged, and mutation is behaviorally inert. |
...
| Artifacts | Every candidate writes versioned `lineage.json`, canonical `genotype/` inputs, and `generation/normalized_candidate.java`; crossover candidates also write `crossover/provenance.json`. Runs write input `config.yaml` and `resolved_config.json`, while remaining metrics/stage outputs retain the transitional flat layout. | Phase 1 state, lineage, provenance, and resolved configuration are implemented; the complete canonical tree, timing, raw-stage evidence, hashes, and schema-aware readers remain later work. |
| LLM logging | Final-generation HTTP attempts write UTC-stamped JSON. Mock generation is not logged. | Mutation/alignment calls and complete per-stage timing are absent. |
