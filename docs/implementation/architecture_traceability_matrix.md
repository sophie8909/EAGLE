...
| MUT-01 | Strategy/Code/Balance boundaries, shared immutable strategy gameplay context, strategy-region-only Code evidence, deterministic reusable-rule deltas, bounded structured retries, and authoritative mutation inputs | mutation | `eagle/mutation.py`, `eagle/rewrite.py`, `eagle/reusable_generation_prompt.py`, `eagle/strategy_reflection.py` | mutation/reflection tests | Implemented |
