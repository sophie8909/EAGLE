...
| MUT-01 | Strategy changes policy from gameplay evidence; Prompt and Code receive only policy, exact Java, and source-matching validation/compiler diagnostics, never gameplay-performance evidence. Prompt changes reusable generation rules, while Code records a diagnosis before directly revising parent Java; all have bounded attempts, immutable domain context, authoritative inputs, and disjoint outputs | mutation | `eagle/mutation.py`, `eagle/rewrite.py`, `eagle/code_reflection.py`, `eagle/strategy_reflection.py` | mutation/reflection tests | Implemented |
