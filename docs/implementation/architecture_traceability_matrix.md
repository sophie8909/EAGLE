...
| EVAL-01 | Compile each validated decoder sample at most once, promote the first success, and run ten distinct opponents × three maps × three rounds × two sides once | evaluation | `eagle/evaluation.py`, `evaluation/match_matrix.py`, `third_party/microrts/src/ai/abstraction/WorkerRush.java` | generation-attempt/evaluation matrix/opponent compile tests | Implemented |
