...
| MUT-01 | Strategy/Code/Prompt Compliance boundaries, immutable gameplay/API compliance context, strategy-region-only Code evidence, deterministic reusable-rule deltas, bounded structured retries, and authoritative mutation inputs | mutation | `eagle/mutation.py`, `eagle/rewrite.py`, `eagle/reusable_generation_prompt.py`, `eagle/strategy_reflection.py` | mutation/reflection tests | Implemented |
| MUT-02 | Standalone repeated reflection inspection fixes one evaluated Worker Rush parent/context, can independently inspect all operators or feed successful Strategy/Code children into Prompt Compliance, preserves every role attempt, and reports expected-versus-actual genotype changes with explicit source provenance | mutation, artifact schema | `eagle/reflection_inspection.py`, `reflection_inspection.sh`, `configs/reflection_inspections/` | reflection-inspection tests plus real artifact audit | Implemented |
