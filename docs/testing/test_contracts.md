...
| Balance Mutation | Aggregate opponent × map × side W/D/L table only → weak-cell analysis → ordered strategy rewrite and exact reusable-rule delta JSON → deterministic generation-prompt rendering → final Java generation; no prompt/Java/raw-trace leakage into the reflector; either rewrite failure preserves both genes; malformed historical marked prompts are replaced at the rewrite boundary |
...
| Opponent-wise lexicase | exactly seven maximized opponent cases; seeded case-order filtering; joint parent-plus-offspring selection without replacement; fixed-size survivor behavior |
| Reflection operator selection | Exactly `static`, `aos_opponent`, and `aos_head2head`; config validation and resume identity; fixed static probabilities with no credit matches; comparison parent follows policy/generation-prompt/Java mutation-evidence provenance after crossover; historical execution-first seven-case rank reward without direct matches; configured head-to-head matrix with `[0,1]` W/D/L reward; shared EMA/floor; common artifacts and mode-labelled analysis |
