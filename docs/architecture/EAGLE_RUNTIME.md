...
The optional independent `watchdog.sh` only polls that entrypoint and starts the
same configured runtime when it is stopped or unhealthy. It does not own a
second server, endpoint, model, or runtime configuration.
