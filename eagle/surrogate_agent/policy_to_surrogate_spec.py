"""Map fixed surrogate policies onto deterministic EAGLE surrogate specs."""

from __future__ import annotations

from .prompt_policy_compiler import Policy, compile_prompt_to_policy


def policy_to_surrogate_spec(policy: Policy) -> dict:
    """Convert a validated fixed policy into the Java surrogate spec format."""
...
def compile_prompt_to_surrogate_spec(prompt: str) -> tuple[Policy, dict]:
    """Compile a prompt into a validated policy and then into a surrogate spec."""

    policy = compile_prompt_to_policy(prompt)
    return policy, policy_to_surrogate_spec(policy)
