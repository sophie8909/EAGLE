...
| EVO-03 | Whole-generation offspring assignment precedes all reflection/rewrite work, which precedes one final Java-materialization phase | evolutionary flow, mutation | `eagle/search.py`, `eagle/resume.py`, `eagle/code_reflection.py` | offspring-planning and Code Reflection phase-order tests | Implemented |
| INIT-01 | Mixed generation zero keeps one configured policy, fills remaining policy genes with independent gameplay-contract-grounded LLM calls, and evaluates one shared fixed Java seed | evolutionary flow, artifact schema | `eagle/initial_population.py`, `eagle/search.py`, `eagle/config.py` | initial-population and prompt-resource tests | Implemented |
| MUT-01 | Strategy changes policy from gameplay evidence; Prompt and Code receive only policy, exact Java, and source-matching validation/compiler diagnostics, never gameplay-performance evidence. Prompt changes reusable generation rules, while Code receives an explicit interface/unit reference, reports strategy fidelity, code simplicity, and game compliance, and revises parent Java only for a reported defect; all have bounded attempts, immutable domain context, authoritative inputs, and disjoint outputs | mutation | `eagle/mutation.py`, `eagle/rewrite.py`, `eagle/code_reflection.py`, `eagle/strategy_reflection.py` | mutation/reflection tests | Implemented |
| MUT-02 | Standalone repeated reflection inspection fixes one evaluated Worker Rush parent/context, runs all three operators independently, preserves every role attempt, and reports expected-versus-actual changes | mutation, artifact schema | `eagle/reflection_inspection.py`, `reflection_inspection.sh`, `configs/reflection_inspections/` | reflection-inspection tests plus real artifact audit | Implemented |
| PROMPT-01 | One executable prompt or immutable reference contract per file under `prompts/` | prompt manifest | `eagle/prompts.py`, `prompts/manifest.toml` | prompt-resource tests | Implemented |
