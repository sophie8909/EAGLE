...
| Balance Mutation | Aggregate opponent × map × side W/D/L table only → weak-cell analysis → ordered strategy and code-generation prompt rewrites → final Java generation; no prompt/Java/raw-trace leakage into the reflector; either rewrite failure preserves both genes |
