...
| MUT-01 | Strategy changes policy, Prompt changes reusable generation rules, and Code directly revises parent Java; all have bounded attempts, immutable domain context, authoritative inputs, and disjoint outputs | mutation | `eagle/mutation.py`, `eagle/rewrite.py`, `eagle/code_reflection.py`, `eagle/strategy_reflection.py` | mutation/reflection tests | Implemented |
