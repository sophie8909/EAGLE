...
| Artifact persistence | `eagle.artifacts`, `eagle.final_test.artifacts` | versioned candidate/run/final-test writers | `resolved_config.json` and schema versions | Current readers consume only the versioned canonical tree; historical runs are outside active runtime support. |
| Configuration loading | `eagle.config`, `eagle.llm_profiles` | `ExperimentConfig.from_file`, role profile loading | YAML experiment config and JSON role topology | Equivalent config keys and endpoint fallbacks are not supported. |
| LLM role routing | `eagle.llm_profiles`, `generation.backend` | `LLMProfile`, `load_role_profiles`, stage-specific backend builders | `experiment_env/config/llm_topology.json` | Archived Ollama/proxy launchers and split runtime LLM agents. |
...
| Analysis and plotting | `eagle.analysis.*`, `scripts/analysis/*` | shared records/objectives/errors plus plotting entrypoints | run artifact schema | Analysis reads current canonical artifacts directly. |
