...
Statuses describe the active repository after Phase 2C Full Mutation Pipeline on 2026-07-15. A path in **Implementation** or **Tests** is repository evidence, not proof that the contract is complete. A path prefixed with `Expected:` in **Artifact** is required output that the current implementation does not necessarily write.
...
| ART-05 | Candidate, stage, LLM-attempt, and match UTC timestamps/monotonic durations follow one timing schema | Spec section 25 and 29.25; owner: artifacts/timing_schema.md | eagle/llm_logging.py; eagle/evaluation.py; eagle/mutation.py; eagle/rewrite.py; evaluation/microrts_runner.py | tests/test_phase2c_mutation_pipeline.py plus later timing tests | Candidate timing includes reflection, rewrite, and generation; match timing remains transitional | Partial | P1 | Phase 2C closes mutation-stage timing; validation, integration, and match timing remain for G-13. |
...
| ART-11 | Validation, compilation, integration, alignment, and objective stages each persist typed result payloads | Spec §21 and §23; owner: [`artifacts/artifact_schema.md`](../artifacts/artifact_schema.md) | `eagle/artifacts.py`; `eagle/evaluation.py`; `evaluation/compiler.py` | `tests/test_phase3_validation.py`; `tests/test_phase3_compilation.py`; `tests/test_phase3_integration.py` | `validation/validation_result.json`, `compilation/compilation_result.json`, `integration/integration_result.json` | ⚠️ Partial | P0 | Validation/compilation/integration typed payloads are now persisted; alignment/objective stage artifacts remain later G-12 work. |
...
| EVAL-11 | Prove the canonical successful warning penalty and cap after the diagnostics boundary. | P0 |
