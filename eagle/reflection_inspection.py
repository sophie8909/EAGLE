...
reflection operator independently from one fixed subject without entering the
evolutionary search loop. Its artifacts are intentionally redundant and
human-oriented: the production mutation artifacts remain intact, while
per-trial indexes and diffs make request/response auditing inexpensive.
"""
...
REFLECTION_TYPES = ("strategy", "prompt", "code")
...
class ReflectionInspectionConfig:
    ...
    def from_file(cls, path: str | Path) -> "ReflectionInspectionConfig":
        ...
        if len(order) != len(REFLECTION_TYPES) or set(order) != set(REFLECTION_TYPES):
            raise ValueError(
                "reflection_order must contain strategy, prompt, and code exactly once."
            )
        return cls(
            name,
            experiment_path,
...
def run_reflection_inspection(
    ...
    *,
    ...
    """Evaluate Worker Rush once, then inspect independent mutations."""
    ...
    try:
        ...
        for reflection_type in inspection.reflection_order:
            ...
            for trial_number in trials:
                context_relative_dir = Path("inputs") / reflection_type
                ...
                print(
                    ...
                    "source=fixed_subject status=started",
                    flush=True,
                )
...
def _render_markdown_summary(
    ...
    *,
    ...
    lines = [
        ...
        f"- 每個 reflection 嘗試次數：`{inspection.trials_per_reflection}`",
    ...
    lines.extend(
        [
            ...
            "## 人工確認順序",
            "",
            "1. 三個 reflection 的固定輸入與 context hash 應相同。",
            "2. 再看 `mutation/<type>_reflection/` 內的 request、response 與 validation/retry artifact。",
            "3. 最後看 `changes/`：Strategy 只能改 policy prompt；Prompt 只能改 code-generation prompt；Code 只能改 reflected Java。",
